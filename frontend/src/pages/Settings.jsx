...
export default function Settings() {
  ...
  useEffect(() => {
    fetch(`${API_URL}/me`, { headers: { Authorization: `Bearer ${token}` } })
      ...
      .then((user) => setProfile({ email: user.sub || "", full_name: user.full_name || "" }))
      .catch(() => {
  ...
  const updateProfile = async (event) => {
    ...
    if (!res.ok) {
      setProfileFeedback({ message: "", error: data.detail || "Impossible de modifier le profil" });
      return;
    }
  ...
  const updatePassword = async (event) => {
    ...
    if (passwords.new_password !== passwords.confirm_password) {
      setPasswordFeedback({ message: "", error: "Les nouveaux mots de passe ne correspondent pas" });
      return;
    }
    ...
    if (!res.ok) {
      setPasswordFeedback({ message: "", error: data.detail || "Impossible de modifier le mot de passe" });
      return;
    }

    setPasswords({ old_password: "", new_password: "", confirm_password: "" });
    setPasswordFeedback({ message: "Mot de passe modifie avec succes", error: "" });
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 md:p-8">
      <div className="max-w-6xl space-y-6">
        ...
        <form onSubmit={updateProfile} className="rounded-2xl border border-gray-100 bg-white p-6 shadow-md">
          <div className="mb-6 flex items-center gap-3">
            <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-indigo-100 text-indigo-700">
              <User size={20} />
            </div>
            <div>
              <h2 className="text-lg font-semibold text-gray-800">Modifier le profil</h2>
              <p className="text-sm text-gray-400">Mettre a jour le nom complet et l'email</p>
            </div>
          </div>
          ...
          <div className="mt-6 flex justify-end">
            ...
            >
              {loading === "profile" && <Loader2 size={16} className="animate-spin" />}
              Mettre a jour
            </button>
          </div>
        ...
        <form onSubmit={updatePassword} className="rounded-2xl border border-gray-100 bg-white p-6 shadow-md">
          <div className="mb-6 flex items-center gap-3">
            <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-indigo-100 text-indigo-700">
              <Lock size={20} />
            </div>
            <div>
              ...
              <p className="text-sm text-gray-400">Saisissez le mot de passe actuel avant d'en definir un nouveau</p>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            {passwordField("Confirmer le nouveau mot de passe", "confirm_password", "confirm")}
          </div>
          ...
          <div className="mt-6 flex justify-end">
            <button
              ...
              className="flex items-center gap-2 rounded-xl bg-indigo-600 px-5 py-2.5 text-sm font-medium text-white hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-70 cursor-pointer"
            >
              {loading === "password" && <Loader2 size={16} className="animate-spin" />}
              Mettre a jour
            </button>
          </div>
        </form>
